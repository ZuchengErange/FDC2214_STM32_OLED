...
{					     
  ...
  GPIO_SetBits(GPIOA,GPIO_Pin_4|GPIO_Pin_5|GPIO_Pin_6|GPIO_Pin_7);				  //PA.4,PA.5 输出高(第一片fdc2214)	PA.2,PA.3 输出高(第二片)
}
...
////    }	 
//} 	    
//读1个字节，ack=1时，发送ACK，ack=0，发送nACK    1
...
//读1个字节，ack=1时，发送ACK，ack=0，发送nACK   2
...
u16 FDC_Read(u8 reg)
{
	u16 res;
   FDC_IIC_Start(); 
	FDC_IIC_Send_Byte((FDC2214_ADDR <<1)|0);//发送器件地址+写命令<<1)|0    
	FDC_IIC_Wait_Ack();		//等待应答 
    FDC_IIC_Send_Byte(reg);	//写寄存器地址
    FDC_IIC_Wait_Ack();		//等待应答
    FDC_IIC_Start();
	FDC_IIC_Send_Byte((FDC2214_ADDR <<1)|1 );//发送器件地址+读命令	 <<1)|1 
    FDC_IIC_Wait_Ack();		//等待应答 
...
u16 FDC_Read1(u8 reg)
{
	u16 res;
   FDC_IIC_Start1(); 
	FDC_IIC_Send_Byte1((FDC2214_ADDR <<1)|0);//发送器件地址+写命令<<1)|0    
	FDC_IIC_Wait_Ack1();		//等待应答 
    FDC_IIC_Send_Byte1(reg);	//写寄存器地址
    FDC_IIC_Wait_Ack1();		//等待应答
    FDC_IIC_Start1();
	FDC_IIC_Send_Byte1((FDC2214_ADDR <<1)|1 );//发送器件地址+读命令	 <<1)|1 
    FDC_IIC_Wait_Ack1();		//等待应答 
...
u32 FCD2214_ReadCH1(u8 index)   //2
{
	u32 result;
	switch(index)
	{
...
//	FDC_GPIO_Init();
	...
	if(res==0x5449)  //res==0x5449
	{
		...
		Set_FDC22141(CLOCK_DIVIDERS_C_CH0,0x20,0x02);//选择在0.01MHz ~ 10MHz的传感器频率
		Set_FDC22141(CLOCK_DIVIDERS_C_CH1,0x20,0x02);//Frefx = Fclk = 43.4MHz/2(2分频)
		Set_FDC22141(CLOCK_DIVIDERS_C_CH2,0x20,0x02);//CHx_REF_DIVIDER=2;CHx_FIN_SEL=2
		Set_FDC22141(CLOCK_DIVIDERS_C_CH3,0x20,0x02);
		...
		Set_FDC22141(CONFIG,0x14,0x01);//激活模式，使用内部振荡器做参考频率，INTB引脚会随状态寄存器更新被置位
	}
	else return 1;
	return 0;
}
